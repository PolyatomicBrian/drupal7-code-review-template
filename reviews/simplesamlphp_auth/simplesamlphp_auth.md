...
be approved for use in production environments.
    ...
    Module categories: Security, Third-party Integration, User Access & Authentication, User Management
    Reported installs: 5,747 sites


## Description

	This module integrates Drupal with SimpleSAMLphp, an implementation of SAML in PHP. It makes it possible for Drupal to communicate with SAML or Shibbolet
	identity providers (IdP) for authenticating users. The resulting Drupal site can effectively act as a SAML or Shibboleth service provider (SP).


## Summary Results

	Recent versions of the module are all marked as alpha.
...
*. Review the module on drupal.org (metadata, issues)
   ...
   Note the 'Project Information'
	...
	- None of these issues appear to cause reason for concern.
